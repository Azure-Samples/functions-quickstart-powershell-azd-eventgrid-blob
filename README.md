...
This template repository contains an Azure Functions reference sample using the Blob trigger with Event Grid source type, written in PowerShell 7.4 and deployed to Azure using the Azure Developer CLI (`azd`). When deployed to Azure the sample uses managed identity and a virtual network to make sure deployment is secure by default. You can control whether a VNet is used in the sample by setting `VNET_ENABLED` to true or false in the AZD parameters.
...
The `Copy-ToProcessedContainer` function uses the Azure PowerShell Storage cmdlets to upload the processed file to the destination blob container using managed identity authentication.
